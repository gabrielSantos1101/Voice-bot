defmodule ArcaneVoice.KV.Interface do
    ...
    cond do
      ...
      true ->
        case validate_pair({key, value}) do
          ...
          {:ok} ->
            Redis.hset("arcane_voice_kv:#{user_id}", key, value)
  ...
  def multiset(user_id, map) when is_map(map) do
    Redis.hset("arcane_voice_kv:#{user_id}", map_to_list(map))
  ...
  def del(user_id, key) do
    Redis.hdel("arcane_voice_kv:#{user_id}", key)
